...
export class FormComponent implements OnInit {
  ...
  public create(): void {
    ...
    .subscribe(
        ...
        swal.fire('Nuevo Profesor', `El profesor ${profesor.nombre} ha sido creado con exito!!`, 'success' )
      },
      err => {
  ...
  public updateProfesor(profesor: Profesor): void{
    this.profesoresService.updateProfesor(this.profesor).subscribe(
      json => {
        this.router.navigate(['/profesor'])
        swal.fire('Profesor Actualizado', `${json.mensaje}: ${json.profesor.nombre} actualizado con exito !`, 'success' )
      },
      err => {
        this.errores = err.error.errors as String[];
        console.error('Codigo del error desde el backEnd: '+ err.status);
